...
// 🚨 徹底拔除 `filter: blur` 以防止 Android GPU 記憶體爆掉
// 只利用 opacity, x 與 scale 製造絲滑果凍滑入感
...
export default function App() {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      .liquid-panel {
        ...
        /* 拔除每一張卡片上的 backdrop-filter，這是 Android 滑動崩潰的元兇！改用單純漸層取代 */
      ...
      /* 保留最上方與最下方 Navbar 的毛玻璃效果，這兩個不影響捲動效能 */
      .gradient-frosted {
  ...
  return (
    <div className="fixed inset-0 text-slate-800 font-sans selection:bg-indigo-100 flex flex-col items-center bg-[#E8EEF5]">
      ...
      {/* 🌟 物理沙盒隔離區：外層 main 鎖死為純觀景窗，完全不負責滾動！ */}
      <main className="w-full max-w-md relative z-10 flex-1 overflow-hidden pt-safe">
        ...
        {/* 🌟 拔除 AnimatePresence，新舊頁面瞬間切換強制掛載，舊記憶體立刻銷毀，100% 杜絕全白 Bug！ */}
                ...
                return (
                  <LiquidRippleNode
                    ...
                    onClick={() => { 
                      ...
                      // 🌟 點擊新日期時，精準回到頂部 (直接操作專屬的 scrollRef)
  ...
  return (
    ...
    >
      <div className="p-5 pt-8 space-y-6 relative z-10">
        ...
        <AnimatePresence>
          {selectedVoucher && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-center justify-center p-5 bg-slate-900/60 backdrop-blur-xl">
